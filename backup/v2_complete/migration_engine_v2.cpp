...
static TaskTypeV2 classify_task(uint32_t util, uint32_t run_queue, uint32_t wakeups) noexcept {
    if (util > TaskThresh::COMPUTE_INTENSIVE && run_queue < 2 && wakeups < 10) {
        return TaskTypeV2::COMPUTE_INTENSIVE;
    }
    if (util > TaskThresh::MEMORY_INTENSIVE && run_queue >= 2 && run_queue < 6 && wakeups >= 10) {
        return TaskTypeV2::MEMORY_INTENSIVE;
    }
    if (util < 256 && run_queue >= 4 && wakeups >= 20) {
        return TaskTypeV2::IO_INTENSIVE;
    }
    if (util < 128 && run_queue < 2) {
        return TaskTypeV2::IO_INTENSIVE;
    }
    return TaskTypeV2::UNKNOWN;
}
...
// 按任务类型选择目标核心
static int select_target_by_task_type_v2(TaskTypeV2 task_type, 
    ...
    for (int i = 0; i < 8; i++) {
        ...
        switch (task_type) {
            case TaskTypeV2::COMPUTE_INTENSIVE:
                if (is_all_big) {
                ...
                break;
            case TaskTypeV2::MEMORY_INTENSIVE:
                if (is_all_big) {
...
MigResult MigrationEngineV2::decide(int cur, uint32_t therm, bool game) noexcept {
    ...
    // ========== 3. 任务分类 ==========
    TaskTypeV2 task_type = classify_task(cur_util, cur_rq, loads_[cur].wakeups);
