...
void EventLoop::process() noexcept {
    ...
    if (is_idle) {
        ...
        // 频率对齐
        cfg.target_freq = freq_mgr_.snap(static_cast<uint32_t>(adjusted_freq), domain_idx);        cfg.target_freq = std::clamp(cfg.target_freq, domain.min_freq, domain.max_freq);
    ...
    // 核间迁移决策
    if (loop_count_ % 5 == 0) {
        auto mig_result = migrator_.decide(cur_cpu, f.thermal_margin, is_game);
        if (mig_result.go) {
            cpu_set_t mask;
            CPU_ZERO(&mask);
    ...
    // 日志输出 (每 20 次循环)
    if (loop_count_ % 20 == 0) {
        LOGI("Freq=%u | Model=%u | FAS=%+d | Idle=%d | Game=%d",
             cfg.target_freq, cfg.target_freq, 0, is_idle ? 1 : 0, is_game ? 1 : 0);
    }
}
...
void EventLoop::adjust(bool increase) noexcept {
    if (increase) {
        period_ms_ = std::max(20, period_ms_ - 10);
    } else {
        idle_count_++;
        if (idle_count_ > 100) {
            period_ms_ = std::min(200, period_ms_ + 5);
        }
    }
...
void EventLoop::start() noexcept {
    ...
    // ✅ 修复：统一类型为 uint32_t    uint32_t fc = 0;
